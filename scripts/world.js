...
logo.src = `modules/${CONSTANTS.MODULE_ID}/styles/ws.svg`;
...
/*
const style = document.createElement("style");
style.textContent = ".journal-entry-page:hover .editor-edit { display: block !important }";
...
function classIfFlag(app, [html]) {
	...
	do {
		if (doc.getFlag("world", CONSTANTS.FLAG)) {
			html.classList.add(CONSTANTS.FLAG);
		}
		...
		handler: () =>
			fetch(getRoute("/setup"), {
				...
				body: JSON.stringify({
					...
					title,
					description,
					background: `modules/${CONSTANTS.MODULE_ID}/content/${id}/scenes/backgrounds/login.webp`,
					nextSession: null,
					action: "editWorld",
				}),
				method: "POST",
...
Hooks.on("renderAdventureImporter", (app, html) => {
	const { adventure } = app;
	if (adventure.getFlag("world", CONSTANTS.FLAG) !== ID) return;

	const importOptions = getImportOptions({
		id: adventure.getFlag("world", CONSTANTS.FLAG),
	...
	async _preStep() {
		...
		if (this.currentStep.id === "adventurePack") {
			await new Promise(resolve => {
				game.packs.get(`${ID}.${CONSTANTS.PACK_ADVENTURE_ID}`).render(true);
			...
			await new Promise(resolve => {
				Hooks.on("renderAdventureImporter", app => {
					if (app.adventure.getFlag("world", CONSTANTS.FLAG)) {
						resolve();
					}
				});
			});
...
Hooks.on("setup", () => {
	const description =
		"This Tour will guide you through importing World Smiths's contents.";
	let title = "Importing an adventure";
	if (ID !== CONSTANTS.FLAG) {
	...
	game.tours.register(
		...
		new ImportTour({
			...
			steps: [
				...
				{
					id: "adventurePack",
					selector: `[data-pack='${ID}.${CONSTANTS.PACK_ADVENTURE_ID}']`,
					title: "Adventure Pack",
					content: "Open the adventure compendium pack to view it's contents.",
				},
				{
					id: "adventureDoc",
					selector: `.compendium[data-pack='${ID}.${CONSTANTS.PACK_ADVENTURE_ID}'] .directory-list`,
				...
				{
					id: "importForm",
					selector: `.adventure-importer.${CONSTANTS.PACK_ADVENTURE_ID} .import-form`,
				...
				{
					id: "import",
					selector: `.adventure-importer.${CONSTANTS.PACK_ADVENTURE_ID} button[type='submit']`,
