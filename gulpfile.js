 ...
 */
const mainFilePath = `main.js`; // MOD 4535992
...
// import type {ModuleData} from `@league-of-foundry-developers/foundry-vtt-types/foundry/common/packages.mjs`; // MOD 4535992
...
const getManifest = () => {
    ...
    const worldPath = path.join(json.root, `world.json`);
    const modulePath = path.join(json.root, `module.json`);
    const systemPath = path.join(json.root, `system.json`);
    if (fs.existsSync(worldPath)) {
        json.file = loadJson(worldPath);
        json.name = `world.json`;
...
//  */
// function buildLess() {
//   return gulp.src(`**/*.less`).pipe(less()).pipe(gulp.dest(`dist`));
// }
...
//  */
// function buildSASS() {
//   return gulp.src(`**/*.scss`).pipe(sass().on(`error`, sass.logError)).pipe(gulp.dest(`dist`));
// }
...
const copyFiles = async() => {
    const statics = [
        `lang`, `languages`, `fonts`, `assets`, `icons`, `templates`, `packs`, `module.json`, `system.json`, `template.json`,
        `actors`, `audio`, `data`, `journals`, `scenes`, `scripts`, `styles`, `world.json`,
    ];
 ...
 */
const buildWatch = () => {
    // gulp.watch(`**/*.ts`, { ignoreInitial: false }, gulp.series(buildTS, bundleModule));
    // gulp.watch(`**/*.ts`, { ignoreInitial: false }, gulp.series(buildTS));
    // gulp.watch(`**/*.less`, { ignoreInitial: false }, buildLess);
    // gulp.watch(`**/*.sass`, { ignoreInitial: false }, buildSASS);
    gulp.watch([
        `fonts`, `lang`, `languages`, `templates`, `*.json`
        `actors`, `audio`, `data`, `journals`, `scenes`, `scripts`, `styles`
    ], { ignoreInitial: false }, copyFiles);
}
...
const clean = async () => {
    ...
    /* MOD 4535992
    // if (fs.existsSync(path.join(`styles/`, `${name}.less`))) {
...
async function packageBuild() {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const moduleJson = JSON.parse(fs.readFileSync('./world.json'));
...
const updateManifest = (cb) => {
    ...
    try {
        ...
        const result = `${repoURL}/releases/download/${manifest.file.version}/world.zip`;
